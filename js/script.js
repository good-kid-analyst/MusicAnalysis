class MusicWordleGame {
    ...
    async fetchNewGame() {
        try {
            ...
            const container = document.getElementById('albumCoverContainer');
            const loader = document.getElementById('albumCoverLoader');
            ...
            this.targetAlbum = data.target_album;
            this.gameId = data.game_id;

            if (this.targetAlbum) {
                // Create new image element to test load
                const testImg = new Image();
                testImg.src = this.targetAlbum.image_url;

                testImg.onload = () => {
                    // If image loads successfully
                    albumCover.src = this.targetAlbum.image_url;
                    albumCover.alt = `${this.targetAlbum.name} cover`;
                    albumCover.style.display = 'block';
                    loader.style.display = 'none';

                    // Add loaded class for any transitions
                ...
                testImg.onerror = () => {
                    ...
                    // albumCover.src = 'path/to/placeholder.jpg';
                    // albumCover.style.display = 'block';
                };
            } else {
                // No cover available
            ...
            document.getElementById('albumCoverContainer').style.display = 'none';
        }
    }
